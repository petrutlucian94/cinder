...
class SmbFsTestCase(test.TestCase):
    ...
    def test_delete_volume(self):
        ...
        with mock.patch('os.path.exists', lambda x: True):
            ...
            self._smbfs_driver._delete.assert_any_call(
                self._FAKE_VOLUME_PATH)
            self._smbfs_driver._delete.assert_any_call(fake_vol_info)
        ...
        with mock.patch.object(image_utils, 'resize_image') as fake_resize:
            if extend_failed:
                self.assertRaises(exception.ExtendVolumeError,
                                  self._smbfs_driver._extend_volume,
                                  self._FAKE_VOLUME, 2)
            else:
                self._smbfs_driver._extend_volume(self._FAKE_VOLUME, 2)
    ...
    def test_copy_volume_from_snapshot(self):
        ...
        self._smbfs_driver._extend_volume = mock.Mock()
        self._smbfs_driver._set_rw_permissions_for_all = mock.Mock()
    ...
    def _test_copy_image_to_volume(self, unsupported_qemu_version=False,
        ...
        with contextlib.nested(
            ...
            if exc:
                self.assertRaises(
                    exc, self._smbfs_driver.copy_image_to_volume,
                ...
                fake_fetch.assert_called_once_with(
                    ...
                    self._smbfs_driver._DISK_FORMAT_VHDX,
                    mock.sentinel.block_size)
                fake_resize.assert_called_once_with(self._FAKE_VOLUME_PATH,
                                                    self._FAKE_VOLUME['size'])
