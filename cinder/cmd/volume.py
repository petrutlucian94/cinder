...
from oslo_service import service as oslo_srv
...
backend_name_opt = cfg.StrOpt(
    'backend_name',
    help='The name of the backend to be managed by this process. It must be '
         'one of the backends defined using the "enabled_backends" option. '
         'Note that normally, this should not be used directly. Cinder '
         'uses it internally in order to spawn subprocesses on Windows.')
...
def _launch_services_win32():
    ...
    if CONF.backend_name and CONF.backend_name not in CONF.enabled_backends:
        msg = ('The explicitly passed backend name "%s" is not among '
               'the enabled backends: "%s".')
        raise exception.Invalid(msg % (CONF.backend_name,
